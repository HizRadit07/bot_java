...
function initializeEntities(gameState) {
  ...
  myCurrentWorm = myPlayer.worms.find(
    (worm) => worm.id === gameState.currentWormId
  );
}
...
function runStrategy() {
  ...
  if (myPlayer.remainingWormSelections > 0) {
    let otherWorms = myPlayer.worms.filter(
      (worm) => worm !== myCurrentWorm && worm.health > 0
    );
...
function digAndMoveTo(destination) {
  let shortestPathCell = findNextCellInPath(
    myCurrentWorm.position,
    destination
  );

  if (
    shortestPathCell.occupier &&
    shortestPathCell.occupier.playerId === myPlayer.id
  ) {
...
function getShootableOpponent(worm) {
  ...
  for (let template of shootTemplates) {
    for (let deltaCoordinate of template.coordinates) {
      ...
      if (
        coordinateIsOutOfBounds(coordinateToCheck) ||
      ...
      if (
        cellToInspect.type === surfaceTypes.DIRT ||
        cellToInspect.type === surfaceTypes.DEEP_SPACE ||
        (cellToInspect.occupier &&
          cellToInspect.occupier.playerId === myPlayer.id)
      ) {
        break;
      }

      let isOccupiedByOpponentWorm =
        cellToInspect.occupier &&
        cellToInspect.occupier.playerId !== myPlayer.id;
...
function getShootTemplates() {
  ...
  for (let direction of directions) {
    ...
    shootTemplates.push({
      name: direction.name,
      coordinates: currentDirectionLine,
    });
  }
